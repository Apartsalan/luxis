...
| AUDIT-H6 | 14-dagenbrief: twee tegenstrijdige betaalinstructies (WIK B2C) — **NB: dit is NIET griffierecht** (apart, juridisch/Documenten, nog open) | Documenten |
...
**Resterend bounded (volgende sessie):** row 59 ("Wettelijke rente"-label hardcoded in `email/incasso_templates.py` r.595/611 bij B2B/handelsrente — klantgerichte tekst, met Lisanne's bewoording) · crash-guards (malformed JWT 500→401, `_determine_direction` None-crash, `distribute_payment` negatief bedrag). **~38 medium resterend** — verifieer elk tegen code.
