...
**Laatst bijgewerkt:** 8 juli 2026 (sessie 187, Opus-bouw + Fable-review — **mailfunctie AFGEMAAKT, blok A+B LIVE + browser-geverifieerd**). Blok A: leesvenster in "Alle e-mails" (gedeeld `EmailDetailPanel`, ook Ongesorteerd), "meer laden" voorbij 200 (infinite query), echte gelezen-status (IMAP `\Seen` + her-sync-update, ongelezen = blauwe stip), sjabloonkiezer altijd zichtbaar met uitleg, ophaalvenster 14→90 dagen. Blok B: tab "Nieuwe aanvragen" (bestaande intake zichtbaar, teller, Maak-dossier/Afwijzen), domein-herkenning opdrachtgevers (vrije/ambigue domeinen uit), "Maak dossier van deze mail" (`POST /api/intake/from-email/{id}`). 2 bestaande bugs gevonden+gefixt: suggesties-500 (`Contact.display_name`→`name`) en `case_number` ontbrak in mail-detail. Vooraf S186-review-fixes (reply draad-wortel + kale AI-mail). Backend 121 mail/intake-tests groen, tsc+build schoon. ⚠️ Openstaand: Ongesorteerd-tab + nieuw-dossier-flow niet apart live doorgeklikt; DMARC ontbreekt. **Next: S188 = die twee live verifiëren, dan heropening werkvoorraad.** Details: SESSION-NOTES S187-entry.
**Vorige (S186):** 8 juli 2026 (Opus + Fable — **mailfunctie: versturen als incasso@ + blok 2 + huisstijl, alles LIVE**). Versturen áls incasso@ via `smtp.basenet.nl:587` (+ zelf IMAP-APPEND naar Verzonden, want BaseNet doet dat niet); bijlage-lek `/compose/send` gedicht; document-send van Gmail-noodroute → provider. Blok 2 LIVE + browser-geverifieerd: dood `/api/email/all` hersteld (6446 mails), zoeken door álle mail (2009 treffers "faillissement"), gesprekketting-fix (References-wortel), beantwoorden/doorsturen (dossier + Ongesorteerd). **Huisstijl-opmaak** (handtekening+logo+schuldhulpblok) onder álle uitgaande mail via `ensure_branded_body`, al-opgemaakte overgeslagen via `already_branded` (prod-render 7/7). 346 tests groen. ⚠️ DMARC ontbreekt (Gmail-spam-risico). **Next = mailfunctie AFMAKEN: `docs/plans/PLAN-mail-afmaken.md` (blok A+B), Opus.** Details: SESSION-NOTES S186-entry.
**Vorige (S185):** 8 juli 2026 (Opus + Fable — uitrol S184 LIVE + nazorg + incasso@ als IMAP onder Lisanne). Details: SESSION-NOTES S185-entry.
**Vorige (S184):** 8 juli 2026 (Opus fix-sprint + Fable-review — 6 audit-punten, via S185 LIVE). Pro-rata credit-fix, verzuim-fix, RLS-gat dicht + opstartcontrole, rolwissel-na-commit, security-regels+rollen. Details: `docs/sessions/S184-MORGEN-CHECKLIST.md`.)
**Vorige:** 8 juli 2026 (sessie 183, Fable — **architectuur+security-audit, read-only. Oordeel livegang: JA, MITS.** Rapport + werkorder: `docs/research/audit-S183-architectuur-security.md`. Nieuw: S183-3 [HOOG] pro-rata-verdeling rekent fout bij creditfacturen (bewezen; 4 zaken in heropeningslijst), S183-1 [MIDDEN] `learned_answers` als enige tabel zonder RLS op prod, S183-2 [MIDDEN] RLS-rolwissel vervalt na tussentijdse commit (31 plekken, live bewezen), S183-4 [LAAG] betaling op/vóór verzuimdatum genegeerd in rente. Aantoonbaar op orde: rekenkernen+tests, auth/OAuth/uploads/secrets, S172-consolidatie AI-laag, scheduler. Next: `docs/sessions/PROMPT-S184.md` — Opus fix-sprint met het rapport als werkorder.)
**Vorige:** 7 juli 2026 (sessie 182, Opus-bouwsprint + Fable-review + backup-migratie — **4 livegang-taken LIVE:** (1) regeling-alarm: gemiste betalingsregeling-termijn → in-app melding (nieuw type `installment_overdue`, alleen actieve regelingen, pending+partial); (2) timeout-regels opgeschoond (4 dode regels op prod gedeactiveerd) + deterministisch (`ORDER BY created_at,id`, filter inactief-doel, warning bij dubbel); (3) opvolg-scan slaat hold/terminale stappen over (voorkomt ruis-muur bij heropening); (4) **getrouwheids-poort** op AI-concepten (dossiernummer + hoofdsom/rente/te-voldoen moeten letterlijk in het concept — anders regenereren max 3×, blijft fout → reviewtaak "⚠ wijkt af"; bijvangst: kapot XXX-vangnet gefixt). **Fable-review** (subagent op Fable-model) vond 2 gaten + 1 achterdeur → alle gefixt: partial-termijn alarmeerde niet, poortwachter kon stil worden, regeling-beëindiging via PATCH sloot termijnen niet. **Backup-migratie VS→EU LIVE**: versleuteld naar Amsterdam, restore-test geslaagd (cases 607/contacts 1168/payments 255 exact live). 21 nieuwe tests, alle geraakte suites groen. Next: `docs/sessions/PROMPT-S183.md` (architectuur+beveiliging-audit op Fable). **Restpunt ~10 juli:** US-bucket wissen na 2 bewezen EU-runs.)
**Vorige:** 7 juli 2026 (sessie 181-F, laatste Fable-dag [→ Fable bleek t/m 12 juli] — **Heropeningsaudit + livegang-spoorboekje, 100% read-only.** Recept voor alle 372 lopende zaken (€1,89M) met voorgestelde stap per zaak (`docs/sessions/S181-werkvoorraad-recept.csv`); heropenen bewezen veilig (0 zaken met stap, auto-draft-vlag uit, niets verstuurt zonder klik). **8 uitvoeringsplannen in `docs/plans/`** (README = rangorde) + **AVG-documentset in `docs/avg/`** + Word-beslispunten voor Lisanne. Nieuw gevonden: gemiste regeling-termijnen onzichtbaar (geen notificatie — URGENT plan, termijnen vanaf 9 juli), dubbele timeout-regel Tweede sommatie, 14-dagenbrief-stap zonder sjabloon, 🔴 **off-site backup staat in de VS én onversleuteld** (migratie geblokkeerd: Backblaze-EU-aanmelding gaf HTTP 400). Creditrente-claim na tegenvraag Arsalan teruggedraaid (verrekening klopt; restfout €59). Wki geldt materieel voor advocaten → AI-getrouwheidspoort gepland. Next: `docs/sessions/PROMPT-S182.md` (Opus-bouwsprint).)
**Vorige:** 6 juli 2026 (sessie 180 — **Betalingen COMPLEET (Fable: onderzoek→bouw→live).** Het "90 cache-only"-gat bleek géén fuzzy-probleem: `cblpcode` op de bankregel IS het dossiernummer; alle 90 zaken matchen op de cent met BaseNet's `cachedpaymentsadmin`. 199 bankregel-betalingen (€152.049) geïmporteerd met hard exact-match-slot per zaak; 47 gecapt op openstaand (rente-nuance S175d, nu volledig betaald). **Eindstand: 255 betalingen op exact de 135 zaken die BaseNet kende — geen betalingen-gat meer.** Bijvangst: 8 "Lopend"-zaken feitelijk voldaan → recept-input. Livegang-rest = mensenwerk (recept/mail/generale repetitie). Next: `docs/sessions/PROMPT-S181.md`.)
**Vorige:** 6 juli 2026 (sessie 179 — **Fase 1b import LIVE (Opus-bouw).** 56 betalingen (€165.697, art. 6:44, workflow-hook UIT voor historie) + 13 betalingsregelingen (121 toekomstige termijnen) op prod; proefzaak IN100215 toont termijn 12 juli (via Lisanne's login geverifieerd), IN100019 vanaf 9 juli. Reconciliatie: 29 "verschillen" = BaseNet's dubbeltelling (geen gemiste betaling); eerlijk gat = 90 zaken met cache-betaling zónder gedateerd record (19 lopend, €33.161) → S180 Fable boekhoud-matching. Klein herstel: Team-tab read-only (kapotte invite-endpoints weg), IN100592 → LegalWork B.V., IN100555 (D-Break, leeg) met rust. 8 nieuwe tests, 168 relevante groen. Next: `docs/sessions/PROMPT-S180.md`.)
**Vorige:** 6 juli 2026 (sessie 178 — **Go-live gap-audit (onderzoek, geen code).** Wat blokkeert de BaseNet-opzegging? Gemeten op XML-export + prod: betalingen fase 1b = WEL nodig maar klein (29/372 lopende zaken, €52.302; ≥5 lijken al voldaan → risico onterecht aanmanen), 12 lopende zaken hebben actieve betalingsregeling (module bestaat, staat leeg; proefzaak IN100215 termijn 12 juli), debiteur-AV-registratie = bewust NIET (361/372 op 2%/mnd contractueel in BaseNet zelf; keten dekt uitzonderingen), "Facturen Legalwork" = impact nihil (1 afgesloten zaak). Prioriteitenlijst in SESSION-NOTES S178-entry. Next: `docs/sessions/PROMPT-S179.md` (Opus — betalingen+regelingen-import).)
**Vorige:** 6 juli 2026 (sessie 177 — **Herstel-sprint + rente-uit-AV, Fable-nacheck→Opus-bouw→Fable-review.** Nacheck S175-dag = alles schoon. LIVE: bijlagen-backfill (5.108 bijlagen/933MB → 2.404 mails, download getest), losse documenten (2.619 → 596 dossiers, Bestanden-tab), dashboard-archief-gaatje dicht, en **taak D: Luxis leest de rente-afspraak zelf uit de AV** (regex 'X% per maand vanaf vervaldag' + Haiku-vangnet; laag dossier>klantkaart>uit-AV>wettelijk; migratie `s177`; frontend-herkomst; AV art. 13.3 = 2%/mnd op alle 8 relaties toegepast). Fable-review: 4 bevindingen gefixt (o.a. AI-intake erfde géén rente — gat van vóór S177). Besluit: rente ALTIJD per factuur vanaf de vervaldatum (werkt al). Detail: SESSION-NOTES S177-entry. Next: `docs/sessions/PROMPT-S178.md` (Fable — go-live gap-audit).)
**Vorige:** 6 juli 2026 (sessie 175 — **Onafhankelijke review S174: GO.** Alle draaiboek-checks doorlopen (diff regel voor regel, 89 tests zelf gedraaid, prod-rooktest: Slim-leren-UI + concept op verweer-dossier + logs + PII-steekproef; relabel-verdeling exact de dryrun, goedgekeurde rijen onaangeraakt). Eén must-fix gevonden + direct live: `get_learned_examples` keek maar naar de 12 nieuwste goedkeuringen → V4-type-matching zou stil falen zodra Lisanne >12 kandidaten goedkeurt; cap → 200 (rode test eerst, commit `5fa4592`, gedeployed). Next: Lisanne's beoordeling begeleiden / K2-meting, `docs/sessions/PROMPT-S176.md`.)
...
| DF122-03 | **M365 email forwarding** — BaseNet forwardt kopieën naar M365, Luxis synct van M365. Parallelle werking: oude zaken op BaseNet, nieuwe op Luxis. Filter zodat ongelinkte mails niet zichtbaar. | ⚪ Vervangen door S185: incasso@ komt binnen via IMAP direct op BaseNet (`imap.basenet.nl`, alleen-lezen, onder Lisanne) — géén forwarding/M365 nodig. |
| S185-MAIL | **Mail-koppeling herkent IN-zaaknummer + opdrachtgever-kenmerk** — nummer-herkenning kende alleen `20xx-xxxxx` (0 van 607 zaken); nu voorrang eigen zaaknummer → kenmerk-kern (`Case.reference`). Grondwaarheidstoets 6.393 archiefmails: 4.407 juist / 4 fout. | ✅ Sessie 185 (`b489e04`) |
| S186-MAIL | **Versturen áls incasso@ + mail-module-doorlichting** — SMTP via BaseNet (`ImapProvider.send_message` = NotImplementedError; globale brug staat nog op test-gmail). Plus: is Luxis een volwaardig mailprogramma (beantwoorden/zoeken/mappen/thread-keten)? Zie `docs/sessions/PROMPT-S186.md`. | 📋 Volgende sessie (start Fable) |
