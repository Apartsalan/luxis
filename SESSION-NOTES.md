...
**Volgende sessie:** Eerst Arsalan-acties afvinken (Sentry gratis account → DSN in `.env` + recreate; Outlook opnieuw koppelen via Instellingen → e-mail; B2-bucket EU-regio bevestigen). Dan (alle drie mét Arsalan): RLS fase 2 + Exact-activatie + FIN-2/CONN-7 afwikkel-wizard. Daarna: AUDIT-FE-1 vervolg (palette-migratie via tones.ts) of E2E-suite die de 6 journeys vastzet. Connectie-audit-polish (CONN-8 t/m 12) is hiermee af; alleen CONN-7+13 resten (beide gepland, niet nu). Lees eerst: `docs/research/connectie-audit.md` + de S160-entry hieronder.
