...
**Laatst bijgewerkt:** 8 juli 2026 (sessie 184, Opus fix-sprint + Fable-review — **alle 6 audit-punten gebouwd op branch `s184-fixes`, NIET gedeployed (wacht op deploy-go).** Pro-rata credit-fix, verzuim-fix, RLS-gat dicht + opstartcontrole (faalt dicht in prod) + drift-guard, rolwissel-na-commit (after_begin-event), --no-cache weg, security-regels+rollen. Fable-review vond 1 must-fix (verzuim-clamp zeroede credit-rente) → gefixt. 1147 suite groen. CLAUDE.md security- + geen-aannames-regel vastgelegd (`743e471`). Next: `docs/sessions/PROMPT-S185.md` — uitrol + nazorg (herrekening 4 zaken, 7 dossiers sluiten). Details: `docs/sessions/S184-MORGEN-CHECKLIST.md`.)
