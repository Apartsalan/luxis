          ...
          <div className="flex h-3 rounded-full overflow-hidden gap-0.5">
            {summary.cases_by_status
              ...
              .map((item) => (
                <div
                  key={item.status}
                  className={`${STATUS_COLORS[item.status] ?? CASE_STATUS_COLOR_FALLBACK} transition-all`}
                  style={{
            ...
            {summary.cases_by_status
              ...
              .map((item) => (
                <div key={item.status} className="flex items-center gap-1.5">
                  <div
                    className={`h-2.5 w-2.5 rounded-full ${STATUS_COLORS[item.status] ?? CASE_STATUS_COLOR_FALLBACK}`}
                  />
            ...
            <div className="rounded-xl border border-border bg-card">
              <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                <div className="flex items-center gap-2">
                  <AlertTriangle className={`h-4 w-4 ${TONES.warning.textMuted}`} />
                  <h2 className="text-sm font-semibold text-card-foreground">
                    Actie nodig
                  </h2>
                </div>
              ...
              <div className="divide-y divide-border">
                ...
                {unlinkedCount > 0 && (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${TONES.warning.chip} ${TONES.warning.ringStrong}`}>
                        Ongesorteerd
                      </span>
                ...
                {summary?.cases_by_status?.some(
                  ...
                  <div className="px-5 py-8 text-center">
                    <CheckCircle2 className={`h-8 w-8 ${TONES.success.textMuted} mx-auto mb-2`} />
    ...
    <div className="rounded-xl border border-border bg-card">
      ...
      <div className="p-5">
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div>
            <p className={`text-2xl font-bold ${TONES.success.text} tabular-nums`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${
                      INVOICE_STATUS_COLORS[inv.status] ?? TONES.gray.chip
                    }`}
                  >
            ...
            >
              {count === 1 ? (
                <button
                  ...
                  className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 border-muted-foreground/30 ${TONES.success.hoverBorder} ${TONES.success.hoverSurface} transition-colors`}
                  title="Markeer als afgerond"
                >
                  {completeTask.isPending && (
                    <div className={`h-2.5 w-2.5 rounded-full ${TONES.success.solid} animate-pulse`} />
                  )}
                </button>
              ) : (
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`inline-flex shrink-0 items-center rounded-full px-2 py-0.5 text-[10px] font-medium ring-1 ring-inset ${TONES.neutral.badge}`}>
              ...
              {isOverdue && (
                <AlertTriangle className={`h-4 w-4 ${TONES.danger.textMuted} shrink-0`} />
              )}
            </div>
          );
        })}
    ...
    <div className="rounded-xl border border-primary/20 bg-primary/5">
      <div className="flex items-center justify-between px-5 py-4 border-b border-primary/10">
        <div className="flex items-center gap-2">
          ...
          <span className={`rounded-md px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider ${TONES.ai.chip}`}>
            AI
          </span>
...
function KycWarningWidget() {
  ...
  return (
    <div className={`rounded-xl border ${TONES.danger.border} ${TONES.danger.surfaceSoft}`}>
      <div className={`flex items-center justify-between px-5 py-4 border-b ${TONES.danger.border}`}>
        <div className="flex items-center gap-2">
          <ShieldAlert className={`h-4 w-4 ${TONES.danger.textMuted}`} />
          <h2 className={`text-sm font-semibold ${TONES.danger.heading}`}>
            WWFT Compliance
          </h2>
          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${TONES.danger.chip}`}>
            {kyc.total_issues} {kyc.total_issues === 1 ? "issue" : "issues"}
          </span>
        </div>
      </div>
      <div className={`divide-y ${TONES.danger.divide}`}>
        {kyc.without_kyc.length > 0 && (
          <div className="px-5 py-3">
            <p className={`text-xs font-medium ${TONES.danger.textStrong} mb-2`}>
            ...
            <div className="flex flex-wrap gap-2">
              {kyc.without_kyc.slice(0, 5).map((item) => (
                <Link
                  ...
                  className={`inline-flex items-center gap-1 rounded-full bg-white px-2.5 py-1 text-xs ring-1 ${TONES.danger.textStrong} ${TONES.danger.ring} ${TONES.danger.hoverSurface} transition-colors`}
                >
              ...
              {kyc.without_kyc.length > 5 && (
                <span className={`text-xs ${TONES.danger.text} self-center`}>
        ...
        {kyc.incomplete.length > 0 && (
          <div className="px-5 py-3">
            <p className={`text-xs font-medium ${TONES.warning.textStrong} mb-2`}>
            ...
            <div className="flex flex-wrap gap-2">
              {kyc.incomplete.slice(0, 5).map((item) => (
                <Link
                  key={item.contact_id}
                  href={`/relaties/${item.contact_id}`}
                  className={`inline-flex items-center gap-1 rounded-full bg-white px-2.5 py-1 text-xs ring-1 ${TONES.warning.textStrong} ${TONES.warning.ring} ${TONES.warning.hoverSurface} transition-colors`}
                >
                  {item.contact_name}
                </Link>
              ))}
          ...
          <div className="px-5 py-3">
            ...
            <div className="flex flex-wrap gap-2">
              {kyc.overdue.slice(0, 5).map((item) => (
                ...
                >
                  {item.contact_name}
                  <span className={`text-[10px] ${TONES.danger.textMuted}`}>
                    ({item.days_overdue}d)
                  </span>
                </Link>
              ))}
...
function ForgottenHoursWarning() {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl border ${TONES.warning.border} ${TONES.warning.surface} px-5 py-3.5`}>
      <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${TONES.warning.iconBox}`}>
        <Clock className={`h-4 w-4 ${TONES.warning.text}`} />
      </div>
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium ${TONES.warning.headingStrong}`}>
          Geen uren geschreven op {dayLabel}
        </p>
        <p className={`text-xs ${TONES.warning.textStrong} mt-0.5`}>
          Vergeten tijd te registreren? Je kunt alsnog uren invoeren.
        </p>
      </div>
      <Link
        href={`/uren?date=${dateStr}`}
        className={`shrink-0 inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium ${TONES.warning.button} transition-colors`}
      >
