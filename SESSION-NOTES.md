...
**Laatst bijgewerkt:** 2 juli 2026 (planning-sessie — Fable/Opus-modelstrategie voor S166 vastgelegd; geen code). Ervóór: 18 juni 2026 (sessie 165 — incasso-backlog 2/3/4 + shadow-learning-fundament live; 0 leer-data want verweer-mails zijn XXX-sjablonen → wacht op BaseNet)
...
**Volgende sessie (S166):** zie `docs/sessions/PROMPT-S166.md` — **BaseNet-import** (voedt shadow-learning) als primair zodra de export er is, + resterende S165-backlog (#1 cliënt-kenmerk, #5 verweer-escalatie, #7 H25). S165 deed backlog 2/3/4 + shadow-learning-fundament. **NIEUW (planning 2 juli):** de S166-prompt bevat nu een Fable/Opus-modelstrategie per stap — assistent zegt bij start welk model + effort te zetten (`/model` + `/effort`). Zie planning-entry hieronder.
