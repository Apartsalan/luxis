...
**Laatst bijgewerkt:** 10 juni 2026 (sessie 161 — security-diepte SEC-1/2/4/5/6/7/8, autonoom op Opus)
**Laatste feature/fix:** Sessie 161 — security-diepte vóór livegang met echte cliënt-PII. **SEC-5 tenant-isolatie = robuust bevonden** (RLS FORCE + WITH CHECK + adversariële test incl. geforceerde cross-tenant read→0 rijen + future-table coverage-guard, én app-laag `tenant_id`-filtering overal — geen lekpad). 4 echte fixes (elk rode→groene test, gedeployed + live geverifieerd): **SEC-7** account-lockout was STIL KAPOT in prod — `authenticate_user` flush'te de teller maar de 401 rolt `get_db` terug → teller bleef altijd 0 (live bewezen: 6× fout login, count 0); nu commit (`2b499fb`). **SEC-1** wachtwoord-wijziging/-reset trekt nu refresh-tokens in (`978eaee`). **SEC-6** upload-grootte gecapt aan de edge (Caddy `request_body 55MB`) + backend pre-read check (`e3124dd`). **SEC-4** npm audit fix (picomatch ReDoS + postcss XSS, `9817da5`) + PyJWT/multipart floors (`c323774`); prod bouwt `--no-cache` → al gepatcht (pip-audit fresh-build 30→5, alle 5 in `pip` zelf). **DEPLOY-BLOKKADE gevonden+opgelost:** VPS had handmatig toegevoegde `mcp.bespoke…`-Caddyblock (nooit gecommit) → elke `git pull` brak af → géén van de fixes was live; mcp-block nu in git (`15ee7b7`), VPS verzoend, alles gedeployed. Zie S161-entry hieronder.
**Openstaande bugs:** **NIEUW uit S161 (laag/hardening, gedocumenteerd):** test-harness `override_get_db` repliceert get_db's commit/rollback NIET → maskeert transactie-semantiek-bugs (zó verstopte de lockout-bug zich) · geen dependency-lockfile (uv.lock) + `--no-cache` met `>=` floors = niet-reproduceerbare builds; voeg pip-audit toe aan CI · app verbindt als superuser (RLS bypassed na mid-request commit; nu alleen PK-refreshes erna, latent) · CSP `script-src 'unsafe-inline'` (Next.js) · VPS-drift: ongecommitte `scripts/disk_guard.sh`+`setup-uptime-monitoring.sh` (S159-hardening) + zwerf-`test_followup.py` (root + backend/app) op VPS. **Voor Arsalan (uit S159):** Outlook 1× opnieuw koppelen (na H3-keywijziging), B2-bucket EU-regio bevestigen. · CONN-7 (=FIN-2 afwikkel-wizard) · CONN-13 (Exact-sync-status) · 4 MEDIUMs (#66 · #70 · #73 · #97) · H6 wacht op Lisanne · 3 Lisanne-vragen derdengelden · FIN-4 Exact payment-sync strippen vóór activatie · AUDIT-FE-1 restant + FE-2 · 2 dev-only Mailpit-fails · ufw filtert Docker-poort 3100 niet.
**Volgende sessie (S162):** zie `docs/sessions/PROMPT-S162.md`. Security-diepte is af; SEC-1/2/4/5/6/7/8 gedekt (SEC-3 in S160). Resteert vooral livegang-finish + de S161-hardening-residuals (CI pip-audit + lockfile; harness-override fixen). **Besluit S160 blijft:** geen autonome AI-incasso-agent. Lees eerst de S161-entry hieronder.
