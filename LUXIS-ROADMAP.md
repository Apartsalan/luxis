...
**Resterend uit audit:** H25 (modules_enabled) · #95 Fernet-key los van SECRET_KEY (vereist token-re-encrypt + deploy-coördinatie — alleen mét Arsalan) · BTW-op-rente (juridisch, samen met #54) · **latent/follow-up:** `update_case` zet `incasso_step_id` nog via `setattr` i.p.v. `move_case_to_step` (3e bypass, buiten #97-audit-scope — case-detail dropdown maakt geen history/log).
