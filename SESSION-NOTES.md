...
**Laatst bijgewerkt:** 10 juni 2026 (sessie 157 — derdengelden-cluster H14–H19 compleet + MEDIUM-triage + H6-advies; autonome nachtsessie)
**Laatste feature/fix:** Sessie 157 — **Derdengelden volledig op orde** (commits `c76f80d`/`aed1c10`/`d371b2b`/`1f66206`): H17 dedup bankimport (IBAN+Volgnr unique, dubbele import = no-op met teller) · H18 overbetaling gecapt op openstaand + per-match savepoints (geen wees-stortingen meer) · H15 storno via tegenboeking (`reversal`-type, vier-ogen op debit-storno's, saldo-guard op deposit-storno, verrekening-storno heropent factuur) · H16 undo van uitgevoerde matches (trust-storno eerst, betaling soft-delete, weer koppelbaar) · H19 'Ongekoppeld'-tab + koppel-dialoog + klikbare KPI-drilldown · H14 vier-ogen als tenant-setting (2+ actieve users ⇒ altijd strikt, Voda 6.22 lid 8; toggle in Instellingen→Kantoor). Extra's: verrekening-bevestigingstaak (wettelijk verplicht, 6.19 lid 5), reject-audit (wie/wanneer/waarom), begunstigde+IBAN verplicht, bankdatum/betalingskenmerk op trust-deposits. Onderzoek vooraf via 3 parallelle sporen (Voda/NOvA primaire bronnen, concurrenten Clio/Smokeball/Actionstep/LEAP/BaseNet, code-audit) → `docs/research/derdengelden-regels.md` (incl. 3 Lisanne-vragen). Browser-geverifieerd: storno-flow, Ongekoppeld-tab, KPI-dialoog, settings-toggle. Daarna **MEDIUM-triage alle 52 rows** (`.audit/TRIAGE-S157.md`): 20 bleken al opgelost, 3 non-issue, **4 gefixt** (`cb9620f`): #61 **16 hardcoded stichtings-IBANs** in e-mailtemplates → tenant-settings met luide placeholder (14-dagenbrief wees zelfs naar kantoorrekening; prod-settings bleken al gevuld) · #52 `/interest` negeerde deelbetalingen (te hoge rente) · #83 verjaring vanaf oudste `claims.default_date` i.p.v. dossier-opening · #69 mod-97 IBAN-validatie. En **H6-advies afgerond** (`3a0bce8`): HR 2016:2704 + lagere rechtspraak geverifieerd, concreet tekstvoorstel (één instructie/één rekening/HR-termijnformulering, `termijn_14_dagen` +15→+16) + 4 beslispunten → `docs/research/14-dagenbrief-advies.md`.
**Openstaande bugs:** 5 echte MEDIUMs uit triage (#66 relatie-validatie · #70 saldo row-lock · #73 bedrag-match op totale schuld via grand_total-cache · #95 Fernet-key los van SECRET_KEY (vereist token-re-encrypt) · #97 verweer-switch via move_case_to_step). H6-implementatie wacht op Lisanne (4 beslispunten). H25 modules_enabled. 3 Lisanne-vragen derdengelden (2e stichtingsbestuurder als approver? · verrekenclausule in opdrachtbevestiging? · debiteuren op stichtingsrekening?). 2 email-router-tests falen alleen lokaal (dev-Mailpit beantwoordt SMTP; CI groen). AUDIT-FE-1 restant + FE-2 onveranderd.
**Volgende sessie:** RLS fase 2 (bewust overgeslagen 's nachts — pooler/connection-string-wijziging zonder toezicht = prod-risico) óf AI Agent fase 4 onderzoek óf H6-implementatie na Lisanne-akkoord.
