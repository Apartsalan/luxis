...
- `frontend/e2e/facturen.spec.ts::F2`, `::F7` — wizard flow + delete-confirm dialog veranderd — **STATUS: BLOCKED** — geblokkeerd door backend-bug (`invoice_lines.btw_percentage` kolom ontbreekt; POST /api/invoices 500). Gedocumenteerd in `tests/UI_BUGS.md` (BUG-001). F2-wizard-UI is up-to-date; F7 zou na backend-migratie de R5-AlertDialog-stijl gebruiken.
