...
**Laatst bijgewerkt:** 10 juni 2026 (sessie 158 — FIN-1 geldstromen-unificatie + FIN-2 archive-guard + 3 audits op Fable)
**Laatste feature/fix:** Sessie 158 — **FIN-1 opgelost** (`8965322`): drie 'geld binnen'-routes deden niet hetzelfde — handmatige 'Via derdengelden'-betaling boekte géén trust-storting. Gedeelde helper `record_trust_debtor_payment()` (collections/service.py) boekt nu altijd trust-deposit én 6:44-betaling samen (gecapt, overschot = saldo); bankimport `execute_match` gerefactored op dezelfde helper. Derdengelden-verrekening kreeg eigen methode `verrekening_derdengelden` (vervuilde voorschotsaldo via `get_advance_balance`). **FIN-2 deels** (`bc445be`): archive-guard — dossier niet archiveerbaar met trust-saldo ≠ 0 of pending transacties (`get_unsettled_reason`, Voda 6.19); afwikkel-wizard nog open (ontwerpkeuze 'klaar'-status nodig — `afgesloten` is in migratie 009 weggemapt, `betaald` mag NIET blokkeren). Daarna op **Fable** drie onderzoeken: **financiele-samenhang.md** (FIN-1…7: hoe betalingen/facturen/derdengelden/Exact samenhangen + Exact-advies: facturen pushen, betalingen NIET (dubbeltelling), stichting nooit in kantoor-Exact) · **connectie-audit.md** (volledige link-graaf + 6 journeys, CONN-1…14: factuur-'overdue' wordt nérgens gezet, vier-ogen zonder notificatie, /intake+/followup onvindbaar in sidebar, tab=betalingen breekt op niet-incasso, rapportages doodlopend, zoeken kent geen facturen) · **readiness-audit.md** (VPS live geverifieerd via SSH: backups+rclone+RLS(46)+fail2ban ✓; **3 blockers: B1 debiteur-PII naar Moonshot/Kimi (AVG), B2 restore nooit getest, B3 deploy zonder alembic+faalt niet hard**; Sentry leeg, uptime-cron logt nooit, #95 = simpele env-var). Modelstrategie vastgelegd in memory: Fable = redeneren/onderzoek, Opus = uitvoering.
**Openstaande bugs:** READY-B1…B3 (blockers vóór echte data) · CONN-1…14 · 5 MEDIUMs triage (#66 · #70 · #73 · #95→TOKEN_ENCRYPTION_KEY zetten · #97) · H6 wacht op Lisanne · 3 Lisanne-vragen derdengelden · FIN-2 wizard (ontwerpkeuze) · FIN-4 Exact payment-sync strippen vóór activatie · AUDIT-FE-1 restant + FE-2 · 2 dev-only Mailpit-fails.
**Volgende sessie (OPUS):** READY-B1 ✅ OPGELOST in S158-staart: KIMI+GEMINI keys uitgeschakeld in /opt/luxis/.env (backup .env.bak-s158), backend ge-recreate, container draait 100% Anthropic, extern /health 200. NB: definitief = keys ook uit code-fallback strippen (kimi_client) zodat niemand ze per ongeluk terugzet. Extra B3-bewijs: deploy.yml health-curl naar localhost:8000 kan in prod NOOIT slagen (poort alleen intern) — check is dubbel kapot. Volgorde: (1) READY-B3 deploy-fix · (3) READY-B2 restore-test · (4) CONN-6 (5 min) + CONN-1 (overdue-job) + CONN-2 (vier-ogen-notificatie) + CONN-3/4/14 (sidebar) + CONN-5 · (5) H1/H2/H3/H5 VPS-hardening. Lees eerst: `docs/research/readiness-audit.md` + `docs/research/connectie-audit.md`. RLS fase 2 + Exact-activatie + FIN-2-wizard = aparte sessies mét Arsalan.
