...
@pytest.mark.asyncio
async def test_bell_hides_classification_but_keeps_email(
    db: AsyncSession, test_user: User, test_tenant: Tenant
):
    """A5/A11 (S198): de meldingenbel verbergt classification_done uit lijst én
    ongelezen-teller. email_received blijft BEWUST zichtbaar — dezelfde lijst voedt
    de dossier-actiefeed (S198-review, Codex #6)."""
    ...
    # Eén verborgen type + twee zichtbare typen.
    ...
    assert NOTIF_EMAIL_RECEIVED in types           # zichtbaar (dossierfeed)
    assert NOTIF_CLASSIFICATION_DONE not in types  # verborgen (pauze)
