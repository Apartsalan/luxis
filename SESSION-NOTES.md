...
**Laatst bijgewerkt:** 3 juli 2026 (sessie 168 — BaseNet-import UITGEVOERD op prod: schone lei + 1168 relaties/607 dossiers/1563 vorderingen + 6393 e-mails + 344 classificaties → 130 verweer-kandidaten (alleen van de 7 vaste opdrachtgevers). Sandwich Fable→Opus→Fable; 2 veiligheidsfixes + 4 kwaliteitsfixes live.)
...
**Volgende sessie (S169):** zie `docs/sessions/PROMPT-S169.md` — **eerst Lisanne de 130 kandidaten laten beoordelen** in "Slim leren" (het menselijke vangnet activeren) vóór verdere heuristiek-tuning. Daarna optioneel: fase 1b (betalingen/contactlinks), F7 backfill-perf. **Uitzoeken: is de prod source-mount (`/opt/luxis/backend/app→/app/app`) bedoeld of dev-override?** (zie deploy-regels-skill S168-correctie + bekende-fouten #28).
