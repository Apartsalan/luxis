...
_CORRECTOR_PROMPT = (
    ...
    "dossierfeiten en het concept-antwoord. Beoordeel streng, maar keur alleen af "
    "wat je aan de meegeleverde stukken kunt aanwijzen.\n\n"
    # S230 (V3): herkalibratie na de S229-feitencheck. De corrector wees in S222 zes
    # "zware fouten" aan; vier daarvan waren aantoonbaar zijn eigen misser (€40,87 en
    # €22,64 wáren de echte openstaande bedragen, IN100370 stond in de onderwerpregel).
    # Deze drie regels sluiten precies die misgrepen uit — zonder de echte vangnetten
    # (verzonnen feiten, toezeggingen) te verzwakken.
    "NIET AFKEUREN (bewezen misgrepen van eerdere controleurs — houd je hieraan):\n"
    "1. BEDRAGEN: het openstaande bedrag uit de dossiergegevens is leidend, PUNT. Staat "
    "het bedrag in de dossiergegevens, dan is `feiten_kloppen` true — ook als het je laag "
    "of onlogisch lijkt naast de facturen of naast een ouder bedrag in de mailwisseling. "
    "Op deels betaalde zaken is het openstaande bedrag veel lager dan de facturen samen; "
    "met rente en kosten kan het juist hoger zijn. Tel NOOIT zelf facturen op om het te "
    "controleren en speculeer niet over 'datafouten'. Een bedrag is alleen verzonnen als "
    "het NERGENS in de dossiergegevens of de mailwisseling voorkomt. Vind je een bedrag "
    "verwarrend zonder toelichting, zet dat in `toelichting` — niet in een afkeuring.\n"
    "2. NAAM EN DOSSIERNUMMER: de eigen kantoornaam (Kesting Legal, mr. L. Kesting, "
    "Amsterdam) is geen dossierfeit maar de afzender zelf — nooit een verzinsel. Een "
    "dossier-/kenmerknummer is een gegeven feit — nooit een verzinsel — als het "
    "voorkomt in de dossiergegevens, in het ONDERWERP van de mail, of in de body. Let "
    "op: het eigen dossiernummer van de zaak (formaat IN1000xx) is per definitie juist; "
    "dat de debiteur of de opdrachtgever daarnaast eigen kenmerken gebruikt, maakt het "
    "onze nummer niet verzonnen.\n"
    "3. TERUGKOPPELING: aankondigen dat een voorstel aan de cliënt wordt voorgelegd en "
    "dat de debiteur bericht krijgt, is GEEN toezegging zolang er expliciet bij staat dat "
    "de betalingsverplichting en de lopende termijnen onverkort blijven gelden. Ook "
    "'laat het tijdig weten als betaling niet lukt' is geen toezegging. Alleen een "
    "bevestigd uitstel, een bevestigde regeling of (gedeeltelijke) kwijtschelding is er "
    "een.\n\n"
    ...
    '  "feiten_kloppen": bool,              // geen verzonnen bedrag/naam/factuurnummer (zie regels 1 en 2 hierboven)\n'
    '  "geen_toezegging": bool,             // geen kwijtschelding/uitstel/regeling/excuses BEVESTIGD (zie regel 3)\n'
...
def _refusal_text(message: str) -> str:
    """Haal de proza-weigering uit de foutmelding terug, voor het rapport."""
    marker = "invalid JSON:"
...
async def _run_case(case: dict, *, corrector: bool) -> dict:
    ...
    try:
        ...
        # S230 (V3b): een niet-JSON antwoord is meestal géén storing maar een
        # weigering in proza ("dit is geen mail van een debiteur"). Dat is gewenst
        # gedrag en mag de score niet als generatie-fout vervuilen. Alleen echte
        # storingen (time-out, geen sleutel, HTTP-fout) blijven "error".
        if "invalid JSON" in str(e):
            return {"id": case["id"], "geweigerd": _refusal_text(str(e))}
...
async def _load_goud(tenant_id: str, limit: int) -> list[dict]:
    ...
    async with async_session() as db:
        ...
        for la in rows:
            ...
            # S230 (V3b): ook de OPDRACHTGEVER mailt inkomend op dezelfde zaak
            # ("hierbij de factuur, neem in behandeling"). Zo'n mail is geen
            # debiteurenvraag; de antwoordroute weigerde hem terecht en dat telde
            # in S222 als "generatie-fout". Sluit het cliënt-domein hier al uit,
            # dan meten we alleen wat de route echt moet kunnen.
            uitgesloten = ["%kestinglegal%"]
            client = await db.get(Contact, case.client_id)
            client_email = (getattr(client, "email", None) or "").strip()
            if "@" in client_email:
                uitgesloten.append(f"%{client_email.split('@')[-1]}")

            vraag_filter = select(SyncedEmail).where(
                ...
                *[SyncedEmail.from_email.notilike(p) for p in uitgesloten],
            )
            question = (
                await db.execute(
                    vraag_filter.order_by(SyncedEmail.email_date.desc()).limit(1)
                )
            ).scalar_one_or_none()
...
def _render_report(rows: list[dict]) -> str:
    ...
    for r in rows:
        ...
        if r.get("error"):
            ...
            continue
        if r.get("geweigerd"):
            lines.append(f"\n**GEWEIGERD (in proza, geen concept):** {r['geweigerd'][:400]}")
            continue
