...
**Laatst bijgewerkt:** 10 juni 2026 (sessie 160 — CONN-8 t/m 12 polish-batch, autonoom op Opus)
**Laatste feature/fix:** Sessie 160 — CONN-8 t/m 12 (connectie-audit polish-restant), elk los gecommit+gepusht, alle 5 browser-geverifieerd. **CONN-8** rapportages drill-down (KPI→/incasso, mini-cards→/zaken//taken//agenda, maandstaven→`/zaken?date_from=&date_to=`, fase-rijen→/incasso) + root-cause: zaken-pagina las filters NIET uit de URL → ook de bestaande dashboard-`?status=`-link filterde stil niet; nu lazy-init uit searchParams + filterpaneel klapt open bij datum-range (`b82901e`). **CONN-9** relatie-detail → 'Facturen van deze klant' (`/facturen?contact_id=`, `d15d56a`). **CONN-10** uren 'Overzicht per dossier' → Factureer-CTA (`/facturen/nieuw?case_id=`, relatie+dossier voorgevuld + openstaande uren klaar voor import, `abb09b1`). **CONN-11** Ctrl+K zoekt nu ook facturen (factuurnr+referentie) + e-mails (onderwerp/afzender) — rode tests eerst→9/9 groen (`24c9893` backend + `64a5481` palette-rendering). **CONN-12** palette quick-actions: Nieuwe factuur/Incasso/Intake/Agenda/Bankimport (`64a5481`). Zie S160-entry hieronder.
**Openstaande bugs:** **Voor Arsalan (uit S159, niet-blokkerend):** H1 Sentry (gratis account → DSN + recreate), Outlook 1× opnieuw koppelen (na H3-keywijziging), B2-bucket EU-regio bevestigen. · CONN-7 (=FIN-2 afwikkel-wizard) · CONN-13 (Exact-sync-status, na Exact-activatie) · 4 MEDIUMs (#66 · #70 · #73 · #97; #95 ✅ via H3) · H6 wacht op Lisanne · 3 Lisanne-vragen derdengelden · FIN-4 Exact payment-sync strippen vóór activatie · AUDIT-FE-1 restant + FE-2 · 2 dev-only Mailpit-fails · ufw filtert Docker-poort 3100 niet (DOCKER-chain-bypass).
**Volgende sessie (S161 = SECURITY-DIEPTE):** zie `docs/sessions/PROMPT-S161.md`. SEC-3 (secrets) is in S160 al gedaan + schoon; doe de rest: SEC-1 auth/JWT, SEC-2 injectie, **SEC-5 tenant-isolatie/IDOR (belangrijkste)**, SEC-6 file-upload, SEC-7 rate-limiting (deels aanwezig — verifieer dekking), SEC-8 exposure (deels al goed), SEC-4 dependencies (npm audit fix + pip-audit). Aanpak: multi-agent security-reviewer fan-out over de echte code → synthese → fixes (rode→groen). **Besluit S160:** autonome AI-incasso-agent NIET bouwen (premortem-onderbouwd, zie `docs/premortem-ai-incasso-agent-2026-06-10.*`); de assistent (AI-concept-berichten, mens beslist) blijft. Focus = finish naar live, geen nieuwe features. Open Arsalan-acties (niet-blokkerend): Outlook opnieuw koppelen, B2-bucket EU-regio (Sentry ✅ gedaan S160). Lees eerst: de S160-entry hieronder.
