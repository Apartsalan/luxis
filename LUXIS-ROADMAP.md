...
**Laatst bijgewerkt:** 10 juni 2026 (sessie 159 — readiness-blockers B1/B2/B3 + CONN-1/2/3/4/5/6/14 + VPS-hardening H2/H3/H5)
...
| FIN-2 | ◐ **Deels (S158, `bc445be`)** Archive-guard: dossier niet archiveerbaar met derdengelden-saldo ≠ 0 of pending trust-transacties (`get_unsettled_reason`, Voda 6.19). **Nog te doen:** geleide afwikkel-wizard (factuurvoorstel → verrekening → restant uitbetalen → sluiten) + pipeline close-step guard — vereist ontwerpkeuze welke 'klaar'-status (twee status-systemen; `afgesloten` weggemapt in 009; `betaald` mag NIET blokkeren). | Derdengelden/Dossier |
...
| CONN-1…14 | 📋 **Connectie-audit (S158, Fable)** — volledige link-graaf + 6 journeys: kern goed verbonden, 14 signalerings-/navigatiegaten. Zwaar: CONN-1 factuur-`overdue` wordt nergens gezet (geen opvolging eigen declaraties) · CONN-2 vier-ogen pending zonder notificatie/taak · CONN-3 /intake onvindbaar (niet in sidebar/palette). Middel: CONN-4 /followup verstopt · CONN-5 notificaties zonder tab-context · CONN-6 `?tab=betalingen` → lege pagina op niet-incasso dossier · CONN-7 geen afwikkelgids (=FIN-2) · CONN-8 rapportages doodlopend. Klein: CONN-9…14. Volgorde + fix-richtingen: `docs/research/connectie-audit.md`. | App-breed |
...
**Resterend uit audit (volgende sessie):** H25 (modules_enabled) · 5 echte MEDIUMs: #66 relatie-validatie (bounded) · #70 saldo row-lock (eigen blokje) · #73 bedrag-match op totale schuld via grand_total-cache (bounded) · #95 Fernet-key los van SECRET_KEY (vereist token-re-encrypt + deploy-coördinatie) · #97 verweer-switch via move_case_to_step (bounded) · BTW-op-rente (juridisch, samen met #54).
