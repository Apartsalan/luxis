...
**Laatst bijgewerkt:** 10 juni 2026 (sessie 159 — 9 taken uit S158-audits: readiness-blockers B1/B2/B3 + CONN-1/2/3/4/5/6/14 + VPS-hardening H2/H3/H5, autonoom op Opus)
**Laatste feature/fix:** Sessie 159 — alle 9 PROMPT-S159-taken af, elk los gecommit+gepusht. **B3** deploy draait nu `alembic upgrade head` + faalt hard op intern+extern health (`3194939`). **B1** Kimi/Gemini volledig uit code → 100% Claude, debiteur-PII niet meer naar Moonshot/China (`4f9879c`, 80 AI-tests groen). **B2** restore-test bewezen (dump→wegwerp-DB, counts exact = live, runbook `docs/runbooks/restore.md`, `d0f4eb7`). **CONN-6** `?tab=`-fallback naar overzicht (`db9b429`), **CONN-1** factuur-overdue-job+notificatie (`ef21ba6`), **CONN-2** vier-ogen-notificatie (`c09b4e2`), **CONN-3/4/14** Intake+Follow-up sidebar (`d119153`), **CONN-5** notif-tab-context (`fe0804a`). **VPS:** H2 uptime-cron gefixt (execute-bit/`/bin/bash`), H3 `TOKEN_ENCRYPTION_KEY` gezet+doorgegeven (`88c9364`), H5 ufw actief (22/80/443). Frontend browser-geverifieerd, backend rood→groen. Zie S159-entry hieronder.
**Openstaande bugs:** **Voor Arsalan:** H1 Sentry (gratis account → DSN + recreate), Outlook 1× opnieuw koppelen (na H3-keywijziging), B2-bucket EU-regio bevestigen. · CONN-7 t/m 13 (polish-batch) · FIN-2 afwikkel-wizard · 4 MEDIUMs (#66 · #70 · #73 · #97; #95 ✅ via H3) · H6 wacht op Lisanne · 3 Lisanne-vragen derdengelden · FIN-4 Exact payment-sync strippen vóór activatie · AUDIT-FE-1 restant + FE-2 · 2 dev-only Mailpit-fails · ufw filtert Docker-poort 3100 niet (DOCKER-chain-bypass).
**Volgende sessie:** Eerst Arsalan-acties afvinken (Sentry gratis account → DSN in `.env` + recreate; Outlook opnieuw koppelen via Instellingen → e-mail; B2-bucket EU-regio bevestigen). Dan: RLS fase 2 + Exact-activatie + FIN-2 afwikkel-wizard (alle drie mét Arsalan), CONN-7 t/m 13 polish-batch, AUDIT-FE-1 vervolg (palette-migratie via tones.ts). Lees eerst: `docs/research/connectie-audit.md` + `docs/research/readiness-audit.md` + de S159-entry hieronder.
