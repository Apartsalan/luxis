...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["incasso_step_id"], ["incasso_pipeline_steps.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["reviewed_by_id"],
            ["users.id"],
    ...
    op.create_index(
        op.f("ix_followup_recommendations_case_id"),
        "followup_recommendations",
        ["case_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_followup_recommendations_tenant_id"),
        "followup_recommendations",
        ["tenant_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        op.f("ix_followup_recommendations_tenant_id"), table_name="followup_recommendations"
    )
    op.drop_index(
        op.f("ix_followup_recommendations_case_id"), table_name="followup_recommendations"
    )
