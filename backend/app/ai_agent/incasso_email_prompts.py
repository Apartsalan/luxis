...
STEP_SPECIFIC_GUIDANCE: dict[str, str] = {
    ...
    "Verweer beantwoorden": (
        ...
        "STAP 3 — Bij match: KOPIEER de body LETTERLIJK.\n"
        "Plak de volledige body van het matchende voorbeeld (zoals hieronder gegeven) "
        "WOORD-VOOR-WOORD in de weerlegging-sectie van het sjabloon. Niet parafraseren, "
        "niet inkorten. Begin met '[OPENING]' = 1 korte zin die letterlijk verwijst "
        "naar wat debiteur zei (bv. 'U heeft gesteld dat het abonnement is opgezegd.'), "
        "gevolgd door de gekopieerde body.\n\n"
        "STAP 4 — Concrete tegenwerping van debiteur? Gebruik AV.\n"
        "Stelt debiteur SPECIFIEK iets tegen ('nee dat is niet zo', 'dat klopt niet', "
        "'er is wel/niet X gebeurd', 'ik heb wel/niet Y gedaan')? Doorzoek de "
        "Algemene Voorwaarden van cliënt (sectie '### Algemene Voorwaarden van "
        "cliënt' hieronder) op het relevante artikel.\n"
        "  - Zoek artikel-nummer + exacte bewoording dat dit verweer weerlegt\n"
        "  - Voeg 1-3 extra zinnen toe NA de gekopieerde library-tekst, "
        "    bijvoorbeeld: 'U stelt voorts dat <claim debiteur>. Op grond van "
        "    artikel <X.Y> van de algemene voorwaarden geldt echter: <citaat AV>. "
        "    Cliënte beroept zich uitdrukkelijk op deze bepaling.'\n"
        "  - Citaat MOET letterlijk uit AV-tekst komen (geen verzinnen)\n"
        "  - Als AV geen specifieke bepaling bevat: skip deze stap, geen verzonnen tekst\n"
        "Doel: laat zien aan wederpartij dat reactie gebaseerd is op contractuele "
        "afspraken, niet op generiek standpunt. Werkt zoals een goede medewerker: "
        "concreet onderbouwen met overeenkomst.\n\n"
        "STAP 5 — ALLEEN bij echt geen library-match: placeholder.\n"
        "Geen enkele library-match betekent: het verweer past in geen van bovenstaande 5 "
        "categorieen EN geen AV-bepaling adresseert het. Pas dan: '[handmatig invullen "
        "door Lisanne: weerlegging van de stelling dat <kernverweer letterlijk uit "
        "incoming_defense>]'. Het kernverweer MOET de exacte bewoording van debiteur "
        "weergeven, geen generiek voorbeeld.\n\n"
