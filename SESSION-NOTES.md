...
Bij het zoeken naar resterende bounded MEDIUMs bleken er meerdere al opgelost of non-issue (audit-caveat "lijst bevat non-issues" bevestigd): #48 maandgrafiek `is_active` (al gefilterd, `reports_service.py:170`), #63 `create_case` nakosten/provisie (al gezet, `service.py:479/488`), #84 verjaring schrikkeldag (gebruikt `relativedelta`; `interest._add_years` heeft `day=28`-fallback), #81 event cross-tenant `case_id` (al geguard via `_validate_links` → 404). **Resterend open + echt maar niet schoon-testbaar:** #93 Exact-sync `float()` op geld (`exact_online/sync_service.py:197/198/277`) — schendt "nooit float voor geld", maar extern API-payload-formaat niet zonder live Exact te verifiëren → bewust niet blind veranderd.
