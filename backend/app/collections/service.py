...
async def get_financial_summary(
    ...
    if bik_override_percentage is not None:
        ...
        # DF138-16: aparte bodem `bik_minimum_fee` voor BIK-percentage. Los van
        # provisie-minimum (`minimum_fee`) — dat veld geldt alleen voor de
        # eigen factuur naar de cliënt, niet voor de vordering aan de debiteur.
        case_min_fee_result = await db.execute(
            select(Case.bik_minimum_fee).where(
                Case.id == case_id, Case.tenant_id == tenant_id
            )
        )
        case_bik_min_raw = case_min_fee_result.scalar()
        case_bik_min = (
            Decimal(str(case_bik_min_raw)) if case_bik_min_raw is not None else Decimal("0")
        )
        if case_bik_min > 0 and bik_exclusive < case_bik_min:
            bik_exclusive = case_bik_min
        bik_btw = Decimal("0")
